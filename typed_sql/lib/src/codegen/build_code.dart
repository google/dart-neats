      ...
      ..fields.add(
        Field(
          (b) => b
            ...
            ..assignment = Code('''[
              ${schema.tables.map((table) => '_\$${table.rowClass.name}._\$table').join(', ')}
            ]
          '''),
        ),
      ),
...
Iterable<Spec> buildTable(ParsedTable table, ParsedSchema schema) sync* {
  ...
  String readFromRowReader(String rowReader, ParsedField field) {
    var readBackingType = switch (field.backingType) {
      ...
      _ => throw UnsupportedError('Unsupported type "${field.typeName}"'),
    };
                ...
                ${rowClass.fields.map((f) => '''
                  (
                    ...
                    overrides: <SqlOverride>[
                      ${f.sqlOverrides.map((o) => 'SqlOverride(dialect: ${literal(o.dialect)}, columnType: ${literal(o.columnType)})').join(',')}
                    ],
                  )
                ''').join(', ')}
              ],
      ...
      ..methods.add(
        Method(
          ...
          ''')
            ...
            ..lambda = true
            ..body = Code(
              'where(($rowInstanceName) => ${rowClass.primaryKey.map((f) => '$rowInstanceName.${f.name}.equalsValue(${f.name})').join(' & ')}).first',
            ),
        ),
      )
      ..methods.add(
      ...
      ..methods.add(
        Method(
          (b) => b
            ...
            ..lambda = true
            ..body = Code('''
              \$ForGeneratedCode.renamedRecord(this, (e) => (${record.fields.map((f) => 'e.$f').join(', ')},))
            '''),
        ),
      )
      ..methods.add(
        Method(
          (b) => b
            ...
            ..lambda = true
            ..body = Code('''
            \$ForGeneratedCode.renamedRecord(query, (e) => (${record.fields.mapIndexed((i, f) => '$f: e.\$${i + 1}').join(', ')},))
          '''),
        ),
      )
      ..methods.add(
      ...
      ..methods.add(
        Method(
          (b) => b
            ...
            ..body = Code('''
            yield* _asPositionalQuery.stream().map((e) => (${record.fields.mapIndexed((i, f) => '$f: e.\$${i + 1}').join(', ')},));
          '''),
        ),
      )
      ..methods.add(
...
extension on ParsedRecord {
  String get type =>
      '({${fields.mapIndexed((i, f) => 'Expr<${typeArg[i]}> $f').join(', ')},})';

  String get returnType =>
      '({${fields.mapIndexed((i, f) => '${typeArg[i]} $f').join(', ')},})';
}
...
String backingExprType(String backingType) {
  return switch (backingType) {
    ...
    _ => throw UnsupportedError('Unsupported backingType: "$backingType"'),
  };
}

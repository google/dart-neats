...
extension TableItemExt on Table<Item> {
  ...
  UpsertSingle<Item> upsert({Expr<int>? id, required Expr<Uint8List> value}) =>
  ...
  /// Returns an [UpsertSingle] statement on which `.execute()` must be
  /// called for the row to be inserted or updated.
  UpsertSingle<Item> upsertValue({int? id, required Uint8List value}) =>
  ...
  Insert<Item> insertValuesMapped<T>(
    ...
    required Uint8List Function(T row) value,
...
extension QueryItemExt on Query<(Expr<Item>,)> {
  ...
  Update<Item> update(
    UpdateSet<Item> Function(
      Expr<Item> item,
      UpdateSet<Item> Function({Expr<int> id, Expr<Uint8List> value}) set,
    )
    updateBuilder,
...
extension ExpressionItemExt on Expr<Item> {
  ...
  Expr<Uint8List> get value =>
      $ForGeneratedCode.field(this, 1, $ForGeneratedCode.blob);
}

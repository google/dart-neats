...
extension TableItemExt on Table<Item> {
  ...
  UpsertSingle<Item> upsert({Expr<int>? id, required Expr<DateTime> value}) =>
  ...
  /// Returns an [UpsertSingle] statement on which `.execute()` must be
  /// called for the row to be inserted or updated.
  UpsertSingle<Item> upsertValue({int? id, required DateTime value}) =>
  ...
  Insert<Item> insertValuesMapped<T>(
    ...
    required DateTime Function(T row) value,
...
extension QueryItemExt on Query<(Expr<Item>,)> {
  ...
  Update<Item> update(
    UpdateSet<Item> Function(
      Expr<Item> item,
      UpdateSet<Item> Function({Expr<int> id, Expr<DateTime> value}) set,
    )
    updateBuilder,
    ...
    (item) => updateBuilder(
      item,
      ({Expr<int>? id, Expr<DateTime>? value}) =>

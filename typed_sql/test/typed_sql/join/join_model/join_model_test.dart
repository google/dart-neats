...
void main() {
  ...
  r.addTest('employees.join(..).on(true)', (db) async {
    final result = await db.employees
        ...
        .select((employee, department) => (employee.name, department.name))
        .fetch();
    check(result).unorderedEquals([
      ('Alice', 'Engineering'),
      ('Alice', 'Sales'),
  ...
  r.addTest('employees.selfJoinFindColleagues', (db) async {
    final result = await db.employees
        ...
        .select((e1, e2) => (e1.name, e2.name))
        .fetch();
    check(result).unorderedEquals([('Alice', 'Charlie'), ('Charlie', 'Alice')]);
  });
  ...
  r.addTest('employees.join(..).on(deptId & DeptName)', (db) async {
    ...
    check(
      result,
    ).unorderedEquals([('Alice', 'Engineering'), ('Charlie', 'Engineering')]);
  });
  ...
  r.addTest('departments.leftJoin(emp).groupBy.count', (db) async {
    final result = await db.departments
        ...
        .select((deptName, rowCount) => (deptName, rowCount))
        .orderBy((deptName, rowCount) => [(deptName, Order.ascending)])
        .fetch();

    check(
      result,
    ).deepEquals([('Engineering', 2), ('Marketing', 1), ('Sales', 1)]);
  });

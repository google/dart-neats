...
extension CompanyDatabaseSchema on Database<CompanyDatabase> {
  static final _$tables = [_$Employee._$table, _$Department._$table];
...
extension TableDepartmentExt on Table<Department> {
  ...
  UpsertSingle<Department> upsertValue({
    ...
    required String name,
    required String location,
  }) => insertValue(departmentId: departmentId, name: name, location: location)
      .onConflict(.primaryKey)
      .update(
        (_, excluded, set) =>
  ...
  Insert<Department> insertValuesMapped<T>(
    ...
    table: this,
    rows: rows,
    mappings: [departmentId, name, location],
  );

...
extension TableItemExt on Table<Item> {
  ...
  UpsertSingle<Item> upsertValue({int? id, required MyCustomTypeAlias value}) =>
  ...
  Insert<Item> insertValuesMapped<T>(
    ...
    required MyCustomTypeAlias Function(T row) value,
...
extension QueryItemExt on Query<(Expr<Item>,)> {
  ...
  Update<Item> update(
    UpdateSet<Item> Function(
      Expr<Item> item,
      UpdateSet<Item> Function({Expr<int> id, Expr<MyCustomTypeAlias> value})
      set,
    )
    updateBuilder,
    ...
    (item) => updateBuilder(
      item,
      ({Expr<int>? id, Expr<MyCustomTypeAlias>? value}) =>
...
extension ExpressionItemExt on Expr<Item> {
  ...
  Expr<MyCustomTypeAlias> get value =>
      $ForGeneratedCode.field(this, 1, MyCustomTypeAliasExt._exprType);
}

...
extension TableItemExt on Table<Item> {
  ...
  UpsertSingle<Item> upsert({Expr<int>? id, required Expr<JsonValue> value}) =>
  ...
  /// Returns an [UpsertSingle] statement on which `.execute()` must be
  /// called for the row to be inserted or updated.
  UpsertSingle<Item> upsertValue({int? id, required JsonValue value}) =>
  ...
  Insert<Item> insertValuesMapped<T>(
    ...
    required JsonValue Function(T row) value,
...
extension ExpressionItemExt on Expr<Item> {
  ...
  Expr<JsonValue> get value =>
      $ForGeneratedCode.field(this, 1, JsonValueExt._exprType);
}

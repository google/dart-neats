...
part of 'unique_boolean_test.dart';
...
extension TableItemExt on Table<Item> {
  ...
  UpsertSingle<Item> upsert({Expr<int>? id, required Expr<bool> value}) =>
  ...
  /// Returns an [UpsertSingle] statement on which `.execute()` must be
  /// called for the row to be inserted or updated.
  UpsertSingle<Item> upsertValue({int? id, required bool value}) =>
  ...
  Insert<Item> insertValuesMapped<T>(
    ...
    required bool Function(T row) value,
...
extension QueryItemExt on Query<(Expr<Item>,)> {
  ...
  /// at-most one row because the [Unique] annotation in [Item].
  ///
  /// Returns a [QuerySingle] object, which returns at-most one row,
  /// when `.fetch()` is called.
  QuerySingle<(Expr<Item>,)> byValue(bool value) =>
      where((item) => item.value.equalsValue(value)).first;
...
extension ItemChecks on Subject<Item> {
  ...
  Subject<bool> get value => has((m) => m.value, 'value');
}

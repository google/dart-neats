...
part of 'unique_zero_real_test.dart';
...
extension TableItemExt on Table<Item> {
  ...
  UpsertSingle<Item> upsert({Expr<int>? id, required Expr<double> value}) =>
  ...
  /// Returns an [UpsertSingle] statement on which `.execute()` must be
  /// called for the row to be inserted or updated.
  UpsertSingle<Item> upsertValue({int? id, required double value}) =>
  ...
  Insert<Item> insertValuesMapped<T>(
    ...
    required double Function(T row) value,
...
extension QueryItemExt on Query<(Expr<Item>,)> {
  ...
  /// Returns a [QuerySingle] object, which returns at-most one row,
  /// when `.fetch()` is called.
  QuerySingle<(Expr<Item>,)> byValue(double value) =>
...
extension ExpressionItemExt on Expr<Item> {
  ...
  Expr<double> get value =>
      $ForGeneratedCode.field(this, 1, $ForGeneratedCode.real);
}

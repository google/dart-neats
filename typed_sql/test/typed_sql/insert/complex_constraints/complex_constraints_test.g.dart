...
extension TableCompositePkItemExt on Table<CompositePkItem> {
  ...
  UpsertSingle<CompositePkItem> upsertValue({
    required int pkA,
    required String pkB,
    required String data,
  }) => insertValue(pkA: pkA, pkB: pkB, data: data)
...
extension InsertCompositePkItemExt on Insert<CompositePkItem> {
  ...
  /// * `.update((compositePkItem, excluded, set) => set(...))` to
...
extension InsertOnConflictCompositePkItemExt
  ...
  ///   * `compositePkItem` an [Expr] representing the existing row in
  ///     the database,
...
extension TableMultiUniqueItemExt on Table<MultiUniqueItem> {
  ...
  /// Returns an [UpsertSingle] statement on which `.execute()` must be
  /// called for the row to be inserted or updated.
  UpsertSingle<MultiUniqueItem> upsertValue({
    int? id,
    required String fieldA,
    required int fieldB,
    required String data,
  }) => insertValue(id: id, fieldA: fieldA, fieldB: fieldB, data: data)
      .onConflict(.primaryKey)
      .update(
        (_, excluded, set) => set(
...
extension InsertMultiUniqueItemExt on Insert<MultiUniqueItem> {
  ...
  /// then the conflict action is triggered:
  /// * `.doNothing()` to skip insertion of the new row, and,
  /// * `.update((multiUniqueItem, excluded, set) => set(...))` to
  ///   update the conflicting row.
  ///
  /// If a row to be inserted violates a constraint other than the one
  /// specified in _conflict target_ then the entire `INSERT` statement
...
extension InsertOnConflictMultiUniqueItemExt
  ...
  /// row is updated using the expressions defined with the
  /// [updateBuilder]. The [updateBuilder] is given 3 parameters:
  ///   * `multiUniqueItem` an [Expr] representing the existing row in
  ///     the database,
  ///   * `excluded` an [Expr] representing the row to be inserted in the
  ///     database, and,
  ///   * `set` a function to specify which fields should be updated and
...
extension TableForeignKeyItemExt on Table<ForeignKeyItem> {
  ...
  /// Returns an [UpsertSingle] statement on which `.execute()` must be
  /// called for the row to be inserted or updated.
  UpsertSingle<ForeignKeyItem> upsertValue({
    int? id,
    required int refPkA,
    required String refPkB,
    required String data,
  }) => insertValue(id: id, refPkA: refPkA, refPkB: refPkB, data: data)
      .onConflict(.primaryKey)
      .update(
        (_, excluded, set) => set(
...
extension InsertForeignKeyItemExt on Insert<ForeignKeyItem> {
  ...
  /// then the conflict action is triggered:
  /// * `.doNothing()` to skip insertion of the new row, and,
  /// * `.update((foreignKeyItem, excluded, set) => set(...))` to
  ///   update the conflicting row.
  ///
  /// If a row to be inserted violates a constraint other than the one
  /// specified in _conflict target_ then the entire `INSERT` statement
...
extension InsertOnConflictForeignKeyItemExt
  ...
  /// row is updated using the expressions defined with the
  /// [updateBuilder]. The [updateBuilder] is given 3 parameters:
  ///   * `foreignKeyItem` an [Expr] representing the existing row in
  ///     the database,
  ///   * `excluded` an [Expr] representing the row to be inserted in the
  ///     database, and,
  ///   * `set` a function to specify which fields should be updated and

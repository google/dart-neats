...
part of 'schema_overrides_test.dart';
...
extension BookstoreSchema on Database<Bookstore> {
  ...
  Table<Book> get booksInStock =>
      $ForGeneratedCode.declareTable(this, _$Book._$table);
...
extension ExpressionAuthorExt on Expr<Author> {
  ...
  /// Get [SubQuery] of rows from the `booksInStock` table which
  /// reference this row.
  ///
...
final class _$Book extends Book {
  ...
  static final _$table = $ForGeneratedCode.tableDefinition(
    tableName: 'books_in_stock',
    columns: <String>['book_id', 'title', 'author_id', 'stock'],
    columnInfo: [
      $ForGeneratedCode.columnDefinition(
        type: $ForGeneratedCode.integer,
        isNotNull: true,
...
extension TableBookExt on Table<Book> {
  ...
  Insert<Book> insertValuesMapped<T>(
    ...
    String? Function(T row)? title,
    required int Function(T row) authorId,
    int Function(T row)? stock,
  }) => $ForGeneratedCode.insertValuesMapped(
    table: this,
    rows: rows,
  ...
  /// Delete a single row from the `booksInStock` table, specified by
  /// _primary key_.
  ///
  /// Returns a [DeleteSingle] statement on which `.execute()` must be
  /// called for the row to be deleted.
...
extension QueryBookExt on Query<(Expr<Book>,)> {
  ...
  /// Lookup a single row in `booksInStock` table using the
  /// `title` field
  ///
...
/// Extension methods for building point queries against the `booksInStock` table.
extension QuerySingleBookExt on QuerySingle<(Expr<Book>,)> {
  /// Update the row (if any) in the `booksInStock` table matching this
  /// [QuerySingle].
  ///
  /// The changes to be applied to the row matching this [QuerySingle] are
  /// defined using the [updateBuilder], which is given an [Expr]
  ...
  /// Delete the row (if any) in the `booksInStock` table matching this [QuerySingle].
  ///
  /// Returns a [DeleteSingle] statement on which `.execute()` must be called
  /// for the row to be deleted. The resulting statement will **not**
  /// fail, if there are no rows matching this query exists.
...
/// Extension methods for expressions on a row in the `booksInStock` table.

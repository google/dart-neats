...
extension TableItemExt on Table<Item> {
  ...
  /// the fields `category`, `data`, `score`,
  ...
  UpsertSingle<Item> upsert({
    ...
    required Expr<JsonValue> data,
    required Expr<int> score,
  }) => insert(id: id, category: category, data: data, score: score)
  ...
  UpsertSingle<Item> upsertValue({
    ...
    required JsonValue data,
    required int score,
  }) => insertValue(id: id, category: category, data: data, score: score)
      .onConflict(.primaryKey)
      .update(
        (_, excluded, set) => set(
          category: excluded.category,
  ...
  Insert<Item> insertValuesMapped<T>(
    ...
    table: this,
    rows: rows,
    mappings: [id, category, data, score],
  );

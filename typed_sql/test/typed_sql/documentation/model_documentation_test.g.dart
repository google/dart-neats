...
part of 'model_documentation_test.dart';
...
extension TableAuthorExt on Table<Author> {
  ...
  UpsertSingle<Author> upsert({
    ...
    required Expr<String> name,
    Expr<int?>? favoriteBookId,
  }) => insert(authorId: authorId, name: name, favoriteBookId: favoriteBookId)
      .onConflict(.primaryKey)
      .update(
        (_, excluded, set) =>
            set(name: excluded.name, favoriteBookId: excluded.favoriteBookId),
      );
  ...
  Insert<Author> insertValuesMapped<T>(
    ...
    int? Function(T row)? favoriteBookId,
  }) => $ForGeneratedCode.insertValuesMapped(
    table: this,
    rows: rows,
    mappings: [authorId, name, favoriteBookId],
  );
...
extension TableBookExt on Table<Book> {
  ...
  Insert<Book> insertValuesMapped<T>(
    ...
    int? Function(T row)? editorId,
    required int Function(T row) stock,
  }) => $ForGeneratedCode.insertValuesMapped(
    table: this,
    rows: rows,
    mappings: [bookId, title, authorId, editorId, stock],
  );
